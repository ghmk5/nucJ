...
public class WebAozoraConverter {
  ...
  // 設定ファイルから読み込むパラメータ
  /**
   * リストページ抽出対象 HashMap<String key, String[]{cssQuery1, cssQuery2}>
   * キーとJsoupのcssQuery(or配列)
   */
  ...
  public static WebAozoraConverter createWebAozoraConverter(String urlString, File configPath) throws IOException {
    ...
    return converter;
    // return converter._convertToAozoraText(urlString, baseUri, fqdn,
    // cachePath);
  }
   ...
   * @param cachePath
   * @param interval
   * @param modifiedExpire
   *          この時間以内のキャッシュを更新分として扱う
   * @param convertUpdated
   *          更新時のみ出力
   * @param convertModifiedOnly
   *          追加更新分のみ変換
   * @param convertModifiedTail
   *          最新話から連続したもののみ変換
   * @param beforeChapter
   *          指定話数のみ変換 0は指定無し
  ...
  public File convertToAozoraText(String urlString, File cachePath, int interval, float modifiedExpire,
      boolean convertUpdated, boolean convertModifiedOnly, boolean convertModifiedTail, int beforeChapter)
      throws IOException {
    this.canceled = false;
    // 日付一覧が取得できない場合は常に更新
    this.updated = true;
    ...
    // 末尾の / をリダイレクトで取得
    urlString = urlString.trim();
    if (!urlString.endsWith("/") && !urlString.endsWith(".html") && !urlString.endsWith(".htm")
        && urlString.indexOf("?") == -1) {
     ...
     * if(extractInfos != null) { if (!extractInfos[0].matches(urlString)) {
     * LogAppender.println("読み込み可能なURLではありません"); return null; } }
     */
      ...
      if (chapterHrefs.size() > 0) {
        ...
        for (String chapterHref : chapterHrefs) {
          ...
          if (chapterHref != null && chapterHref.length() > 0) {
            ...
            File chapterCacheFile = new File(
                cachePath.getAbsolutePath() + "/" + chapterPath + (chapterPath.endsWith("/") ? "index.html" : ""));
            ...
            if (!loaded) {
              if (this.modifiedExpire > 0 && (this.convertModifiedOnly || this.convertUpdated)
                  && chapterCacheFile.lastModified() >= expire) {
                LogAppender.append("[" + (chapterIdx + 1) + "/" + chapterHrefs.size() + "] " + chapterHref);
  ...
  private HashSet<String> createNoUpdateUrls(File updateInfoFile, String urlString, String listBaseUrl,
    ...
    if (hrefs == null || updates == null || hrefs.size() != updates.size()) {
      ...
      if (updateInfoFile.exists()) {
        // 前回の更新情報を取得して比較
        BufferedReader updateBr = new BufferedReader(
            new InputStreamReader(new FileInputStream(updateInfoFile), "UTF-8"));
        try {
          String line;
    ...
    if (contentsUpdate != null || updates != null) {
      // ファイルに出力
      BufferedWriter updateBw = new BufferedWriter(
          new OutputStreamWriter(new FileOutputStream(updateInfoFile), "UTF-8"));
      try {
   ...
   *          一覧側で取得したタイトル
   */
  private void docToAozoraText(BufferedWriter bw, Document doc, boolean newChapter, String listSubTitle,
      String postDate) throws IOException {
    Elements contentDivs = getExtractElements(doc, this.queryMap.get(ExtractId.CONTENT_ARTICLE));
    if (contentDivs == null || contentDivs.size() == 0) {
      ...
      if (subTitle == null)
        subTitle = listSubTitle; // 一覧のタイトルを設定
      if (subTitle != null) {
        bw.append("［＃ここから中見出し］\n");
      ...
      if (coutentUpdate != null && coutentUpdate.length() > 0)
        postDate = coutentUpdate;
      if (postDate != null) {
      ...
      if (preambleDivs != null) {
        ...
        bw.append("\n");
        for (Element elem : preambleDivs)
          printNode(bw, elem, startElement, endElement, true);
        bw.append("\n\n");
        bw.append("［＃ここで小さな文字終わり］\n");
        bw.append("［＃ここで字上げ終わり］\n［＃ここで字下げ終わり］\n");
        bw.append("［＃区切り線］\n");
  ...
  private void printImage(BufferedWriter bw, Element img, File imageOutFile) throws IOException {
    ...
    // なろうhttps移行後の画像URLと画像ファイル保存パス生成
    // 移行後のimg srcにはプロトコル指定がなく、//から始まっているものと仮定
    String protocol = this.baseUri.substring(0, this.baseUri.indexOf("//"));
    src = protocol + src;
    int idx = src.indexOf("//");
    if (idx > 0) {
    ...
    // なろうhttps移行前の画像URLと画像ファイル保存パス生成(1.1.0b46オリジナル)
    // int idx = src.indexOf("//");
    // if (idx > 0) {
    // imagePath = CharUtils.escapeUrlToFile(src.substring(idx + 2));
    // } else if (src.charAt(0) == '/') {
  ...
  private void printText(BufferedWriter bw, String text) throws IOException {
    ...
    for (char ch : chars) {
      ...
      switch (ch) {
      case '《':
        bw.append("※［＃始め二重山括弧、1-1-52］");
        break;
      case '》':
        bw.append("※［＃終わり二重山括弧、1-1-53］");
        break;
      case '［':
        bw.append("※［＃始め角括弧、1-1-46］");
        break;
      case '］':
        bw.append("※［＃終わり角括弧、1-1-47］");
        break;
      case '〔':
        bw.append("※［＃始め亀甲括弧、1-1-44］");
        break;
      case '〕':
        bw.append("※［＃終わり亀甲括弧、1-1-45］");
        break;
      case '｜':
        bw.append("※［＃縦線、1-1-35］");
        break;
      case '＃':
        bw.append("※［＃井げた、1-1-84］");
        break;
      case '※':
        bw.append("※［＃米印、1-2-8］");
        break;
  ...
  /**
   * Element内のinnerHTMLを取得 間にあるダグは無視 置換設定があれば置換してから返す
   */
      ...
      if (extractInfo.idx == null) {
        ...
        for (int i = 0; i < extractInfo.idx.length; i++) {
          if (elements.size() > extractInfo.idx[i]) {
            ...
            if (pos < 0)
              pos = elements.size() + pos;// 負の値なら後ろから
  ...
  private boolean cacheFile(String urlString, File cacheFile, String referer) throws IOException {
    ...
    if (cookie != null && cookie.length > 0)
      conn.setRequestProperty("Cookie", cookie[0].query);
    if (referer != null)
